...
FORMAL_PROGRESS=1/9≈11%
CURRENT_BLOCKER=NONE
LAST_CLOSED_TASK=S0-R2-LITERATURE-CORRECTION-AND-S0-CLOSEOUT-AUDIT-R2
LAST_CLOSED_TASK_END_HEAD=a09cd6b2e9ea83669d54392b170a449a0716a356
```
...
当前授权任务为 `S1-R0-UPSTREAM-REPRO-CONTRACT-ENV-SMOKE-R1`。本任务只冻结原版复现合同并验证环境、短训练和评测链路；正式长训练仍未授权。
...
```text
UNIQUE_NEXT_TASK=S1-R0-UPSTREAM-REPRO-CONTRACT-ENV-SMOKE-R1
```

当前任务只允许原版 `ipp-marl` 复现合同、隔离环境和轻量 smoke。`FORMAL_LONG_TRAINING_AUTHORIZED=false`，完成后必须等待总控决定。
